...
### mac is not able to use raster function in the magick package
...
##############################################################################
### Figure D: expressed # of Or/Ir/Gr at single cell resolution
### Histogram

dim(receptorRNA.data)

receptorRNA.data = neuron.batchCorrect@assays$RNA@data[receptorExpSortL[(! receptorExpSortL %in% coReceptorL)],]
receptorRNA.data <- as.data.frame(t(as.matrix(receptorRNA.data)))
receptorRNA.namesL <- colnames(receptorRNA.data)

head(receptorRNA.data)

receptorRNA.data$nor1Genes <-
  apply(receptorRNA.data, 1, function(input){
    # print(input)
    genesL=receptorRNA.namesL[as.numeric(input) >= 2] # cutoff for positive expression # need to check the distribution
    # print(class(genesL))
    # print(genesL)
    # print(as.character(genesL))
    if ( length(genesL) >= 1 ) {
      # print(input)
      print(genesL)
      print(paste(genesL, collapse=','))
      print(class(
        print(paste(genesL, collapse=','))
      ))
      return(paste(genesL, collapse=','))
    } 
    else {return(NA)}
  })
